...
def ensamble_png():
    ...
    out_dir_ = []
    for i in range(0,5):
        out_dir_[i] = params.out_dir + params.ensemble_dir + '_k%d'%(i+1)
    ...
    for i in range(len(names)): 
        p = []
        average = np.zeros((CARVANA_H,CARVANA_W),np.unit8)
        for j in range(0,5):
            p[j] = cv2.imread(out_dir_[i]+'/submit/test_mask/%s.png'%(names[i]))
            p[j] = p[j].astype(np.unit8)
        ...
        prob = cv2.resize(average,dsize=(CARVANA_WIDTH,CARVANA_HEIGHT),interpolation=cv2.INTER_LINEAR)
        mask = prob>127
