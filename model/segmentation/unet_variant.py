...
class UNet1024_post_train_02 (nn.Module):
    def __init__(self, in_shape):
        super(UNet1024_post_train_02, self).__init__()
        C,H,W = in_shape
        #assert(C==3)
...
class UNet1024_GCN_k15_06 (nn.Module): #Large kernel matters
    def __init__(self, in_shape):
        super(UNet1024_GCN_k15_06, self).__init__()
        C,H,W = in_shape
        #assert(C==3)
...
class UNet1024_ASPP_07 (nn.Module):
    def __init__(self, in_shape):
        super(UNet1024_ASPP_07, self).__init__()
        C,H,W = in_shape
        #assert(C==3)
        ...
        self.aspp_down5 = ASPP(256, 256, 256, [6,12,18], [6,12,18])
        self.down5 = StackEncoder(256,  512, kernel_size=3)   # 32
        self.aspp_down6 = ASPP(512, 512, 512, [6,12,18], [6,12,18])
        self.down6 = StackEncoder(512,  768, kernel_size=3)   # 16
        ...
        # x_big_channels, x_channels, y_channels
        self.up6 = StackDecoder( 768, 768, 512, kernel_size=3)  # 16
        self.aspp_up6 = ASPP(512, 512, 512, [6,12,18], [6,12,18])
        self.up5 = StackDecoder( 512, 512, 256, kernel_size=3)  # 32
        self.aspp_up5 = ASPP(256, 256, 256, [6,12,18], [6,12,18])
        self.up4 = StackDecoder( 256, 256, 128, kernel_size=3)  # 64
        self.up3 = StackDecoder( 128, 128,  64, kernel_size=3)  #128
    ...
    def forward(self, x):
        ...
        out = self.aspp_down6(out)   #;print('down5',down5.size())  #16
        down6,out = self.down6(out)   #;print('down6',down6.size())  #8
        #pass                          #;print('out  ',out.size())

        out = self.center(out)
        out = self.up6(down6, out)
        out = self.aspp_up6(out)
        out = self.up5(down5, out)
        out = self.aspp_up5(out)
        out = self.up4(down4, out)
        out = self.up3(down3, out)
...
class UNet1024_ASPP_08 (nn.Module):
    def __init__(self, in_shape):
        super(UNet1024_ASPP_08, self).__init__()
        C,H,W = in_shape
        #assert(C==3)
...
if __name__ == '__main__':
    ...
    if 1: # BCELoss2d()
        ...
        net = UNet1024_ASPP_05(in_shape=(C,H,W)).cuda().train()
